      ...
      {!user.isLoggedIn ? (
        <div className="p-4 sm:p-5 rounded-xl bg-[#161b22] border border-emerald-500/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
          <div className="flex items-center space-x-3.5">
            <div className="w-10 h-10 rounded-lg bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center flex-shrink-0 text-emerald-400 shadow-sm">
              <Mail className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-sm sm:text-base font-bold text-white">
                Sign in with your Personal Email to initialize your real-time learning ledger
              </h2>
              <p className="text-xs text-slate-400 mt-0.5">
                Use your own email ID (Gmail, Outlook, Yahoo, Proton, or work email). Each email maintains an independent, persistent progress ledger starting at 0 XP.
              </p>
            </div>
          </div>
          <button
            onClick={onOpenGoogleAuth}
            className="px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs font-mono transition-all flex items-center space-x-2 whitespace-nowrap shadow-md shadow-emerald-500/20 active:scale-95"
            id="dashboard-email-login-cta-btn"
          >
            <Mail className="w-3.5 h-3.5" />
            <span>Sign In with Personal Email</span>
